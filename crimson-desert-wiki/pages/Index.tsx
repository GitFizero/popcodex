...
import { tr } from '@/crimson-desert-wiki/context/I18nContext';
...
const Index = () => {
  ...
  const featureCards = useMemo(() => [
    { title: t('featured.story.title'), desc: t('featured.story.desc'), link: '/story', gradient: 'linear-gradient(135deg, hsl(var(--bg-card)) 0%, hsl(var(--crimson-deep)/0.3) 100%)', image: pailuneRuins },
    { title: t('featured.characters.title'), desc: t('featured.characters.desc'), link: '/characters', gradient: 'linear-gradient(135deg, hsl(var(--bg-card)) 0%, hsl(var(--smoke-mid)/0.3) 100%)', image: hernandStreet },
    { title: t('featured.world.title'), desc: t('featured.world.desc'), link: '/world', gradient: 'linear-gradient(135deg, hsl(var(--bg-card)) 0%, hsl(var(--gold-dark)/0.3) 100%)', image: mountainCastle },
  ], [t]);
  ...
  return (
    <main id="main-content" className={`relative z-10 ${shake ? 'animate-[shake_0.3s_ease-in-out]' : ''}`}>
      ...
      {/* Announcement — below fold, deferred */}
      <RevealOnScroll>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mt-8">
            {characters.slice(0, 3).map((char, i) => (
              <RevealOnScroll key={char.id} stagger={i * 150}>
                <Link to="/characters" className="block group" title={`${char.name} — Crimson Desert`}>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mt-8">
            {featureCards.map((card, i) => (
              <RevealOnScroll key={card.title} stagger={i * 150}>
                <Link to={card.link} className="block group" title={`${card.title} — Crimson Desert Wiki`}>
          ...
          <div className="grid grid-cols-2 sm:flex gap-3 sm:gap-4 mt-8 overflow-x-auto pb-4 scrollbar-thin sm:snap-x sm:snap-mandatory">
            {recentLore.map((entry, i) => (
              <RevealOnScroll key={entry.id} stagger={i * 100}>
                <Link to="/lore" className="block group" title={`${entry.title} — Crimson Desert Lore`}>
      ...
      <section className="relative z-10 py-16 content-defer" aria-label={t('world.continent')}>
        <RevealOnScroll>
          <div className="relative h-[400px] overflow-hidden bg-surface border-y border-border">
            ...
            {regions.map(region => (
              <Link key={region.id} to="/world" className="absolute group" style={{ left: `${region.mapPosition.x}%`, top: `${region.mapPosition.y}%`, transform: 'translate(-50%,-50%)' }} title={`${region.name} — Crimson Desert`}>
      ...
      <section className="relative z-10 py-16 px-4 content-defer" aria-label={t('home.latest_articles')}>
        <div className="max-w-6xl mx-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mt-8">
            {blogArticles.slice(0, 3).map((article, i) => (
              <RevealOnScroll key={article.slug} stagger={i * 150}>
                <Link to={`/blog/${article.slug}`} className="block group" title={tr(article.title, lang)}>
          ...
          <RevealOnScroll>
            <div className="flex justify-center mt-8">
              <Link to="/blog" className="btn-outline">{lang === 'fr' ? 'Voir tous les articles' : lang === 'es' ? 'Ver todos los artículos' : lang === 'it' ? 'Vedi tutti gli articoli' : 'View all articles'} →</Link>
            </div>
          </RevealOnScroll>
        </div>
      </section>
