...
const weaponsData: { id: string; name: L4; desc: L4; combos: string[] }[] = [
  { id: 'sword', name: { fr: 'Épée longue', en: 'Longsword', es: 'Espada larga', it: 'Spada lunga', ko: '장검' }, desc: { fr: 'Arme de mêlée équilibrée. Enchaîne des combos dévastateurs avec des transitions fluides.', en: 'Balanced melee weapon. Chain devastating combos with fluid transitions.', es: 'Arma cuerpo a cuerpo equilibrada. Encadena combos devastadores con transiciones fluidas.', it: 'Arma da mischia equilibrata. Concatena combo devastanti con transizioni fluide.', ko: '균형 잡힌 근접 무기. 유려한 전환으로 강력한 콤보를 연결합니다.' }, combos: ['R1 → R1 → R2', 'R1 → R1 → R1 → R2', 'Hold R2 → R1'] },
  { id: 'lance', name: { fr: 'Lance', en: 'Spear', es: 'Lanza', it: 'Lancia', ko: '창' }, desc: { fr: "Longue portée, contrôle l'espace. Excellente pour punir les ennemis trop agressifs.", en: 'Long range, space control. Excellent for punishing overly aggressive enemies.', es: 'Largo alcance, control del espacio. Excelente para castigar enemigos demasiado agresivos.', it: 'Lungo raggio, controllo dello spazio. Eccellente per punire nemici troppo aggressivi.', ko: '긴 사거리, 공간 제어. 과도하게 공격적인 적을 처벌하는 데 탁월합니다.' }, combos: ['R1 → R1 → R2', 'Hold R1 → R2', 'R2 → R2'] },
  { id: 'bow', name: { fr: 'Arc', en: 'Bow', es: 'Arco', it: 'Arco', ko: '활' }, desc: { fr: 'Combat à distance de précision. Chargez les tirs pour des dégâts dévastateurs.', en: 'Precision ranged combat. Charge shots for devastating damage.', es: 'Combate a distancia de precisión. Carga los disparos para daño devastador.', it: 'Combattimento a distanza di precisione. Carica i colpi per danni devastanti.', ko: '정밀 원거리 전투. 차지 샷으로 강력한 데미지를 입힙니다.' }, combos: ['Aim → R2', 'R1 → R1 → R1', 'Hold R2 (Charged)'] },
  { id: 'fists', name: { fr: 'Mains nues', en: 'Bare Fists', es: 'Puños', it: 'Pugni', ko: '맨손' }, desc: { fr: 'Puissance brute. Prises de lutte et coups brisant les armures.', en: 'Raw power. Wrestling grabs and armor-breaking strikes.', es: 'Poder bruto. Agarres de lucha y golpes que rompen armaduras.', it: 'Potenza bruta. Prese di lotta e colpi che spezzano armature.', ko: '순수한 힘. 레슬링 그랩과 방어구 파괴 공격.' }, combos: ['R1 × 4', 'R2 (Grab)', 'R1 → R1 → R2'] },
  { id: 'robot', name: { fr: 'Robot de guerre', en: 'War Robot', es: 'Robot de guerra', it: 'Robot da guerra', ko: '전쟁 로봇' }, desc: { fr: 'Combinaison de combat mécanisée. Dégâts de zone dévastateurs au prix de la mobilité.', en: 'Mechanized combat suit. Devastating AoE damage at the cost of mobility.', es: 'Traje de combate mecanizado. Daño de área devastador a costa de movilidad.', it: 'Tuta da combattimento meccanizzata. Danni ad area devastanti a scapito della mobilità.', ko: '기계화 전투 슈트. 기동성을 대가로 강력한 광역 피해를 줍니다.' }, combos: ['R1 → R2', 'Hold R2 (Beam)', 'L1 + R2 (Barrage)'] },
  { id: 'dragon', name: { fr: 'Dragon', en: 'Dragon', es: 'Dragón', it: 'Drago', ko: '드래곤' }, desc: { fr: 'Monture et combat aérien. Pluie de destruction depuis les cieux.', en: 'Mount and aerial combat. Rain destruction from the skies.', es: 'Montura y combate aéreo. Lluvia de destrucción desde los cielos.', it: 'Cavalcatura e combattimento aereo. Pioggia di distruzione dai cieli.', ko: '탈것과 공중 전투. 하늘에서 파괴의 비를 내립니다.' }, combos: ['R1 (Breath)', 'R2 (Dive)', 'L1 + R1 (Barrel Roll)'] },
];

const bossesData: { name: string; location: L4; difficulty: number; mechanic: L4; status: string; image: string }[] = [
  { name: 'Staglord', location: { fr: 'Déméniss', en: 'Demeniss', es: 'Déméniss', it: 'Déméniss', ko: '데메니스' }, difficulty: 4, mechanic: { fr: 'Charges dévastatrices et invocations de cerfs', en: 'Devastating charges and stag summons', es: 'Cargas devastadoras e invocaciones de ciervos', it: 'Cariche devastanti ed evocazioni di cervi', ko: '강력한 돌진과 사슴 소환' }, status: 'DOCUMENTED', image: bossStag },
  { name: 'Queen Stoneback Crab', location: { fr: 'Pailune', en: 'Pailune', es: 'Pailune', it: 'Pailune', ko: '파일룬' }, difficulty: 3, mechanic: { fr: 'Carapace destructible, attaques sismiques', en: 'Destructible shell, seismic attacks', es: 'Caparazón destructible, ataques sísmicos', it: 'Guscio distruttibile, attacchi sismici', ko: '파괴 가능한 껍질, 지진 공격' }, status: 'DOCUMENTED', image: bossCrab },
  { name: 'White Horn', location: { fr: 'Pics du Nord', en: 'Northern Peaks', es: 'Picos del Norte', it: 'Picchi del Nord', ko: '북부 봉우리' }, difficulty: 5, mechanic: { fr: 'Tempêtes de neige et rage berserk', en: 'Snowstorms and berserk rage', es: 'Tormentas de nieve y furia berserker', it: 'Tempeste di neve e furia berserk', ko: '눈보라와 광폭화' }, status: 'DOCUMENTED', image: bossWhiteHorn },
  { name: 'Reed Devil', location: { fr: 'Marais de Pywel', en: 'Pywel Marshes', es: 'Pantanos de Pywel', it: 'Paludi di Pywel', ko: '파이웰 습지' }, difficulty: 3, mechanic: { fr: 'Embuscades soniques et illusions', en: 'Sonic ambushes and illusions', es: 'Emboscadas sónicas e ilusiones', it: 'Imboscate soniche e illusioni', ko: '음파 매복과 환영' }, status: 'DOCUMENTED', image: enemyReedDevil },
];

const docLabel: L4 = { fr: '📋 DOCUMENTÉ', en: '📋 DOCUMENTED', es: '📋 DOCUMENTADO', it: '📋 DOCUMENTATO', ko: '📋 문서화됨' };
...
const CombatPage = () => {
  ...
  const levelLabel: L4 = { fr: 'Niveau', en: 'Level', es: 'Nivel', it: 'Livello', ko: '레벨' };
