...
export default async function FablePage({ params }: Props) {
  ...
  if (section === 'blog' && path?.[1]) {
    ...
    if (article) {
      const articleJsonLd = generateWikiArticleJsonLd(FRANCHISE_ID, article, locale, 'Fable Fan Wiki');
      ...
      ssrContent = (
        <SSRArticleContent
          ...
          coverAlt={tr(article.title, locale)}
    ...
    ssrContent = (
      <SSRSectionContent wikiClass={WIKI_CLASS} title={tr((seo as any).blog?.title, locale)} description={tr((seo as any).blog?.desc, locale)}>
        <div>
