...
const Footer = memo(() => {
  const { toggleNoirMode, noirMode } = useApp();
  const { t } = useI18n();
    ...
    <footer className="relative z-10 bg-surface border-t border-border" role="contentinfo">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mt-8">
          <div>
            <h3 className="font-display text-lg text-gold-bright mb-3">🐺 WLV WIKI</h3>
          ...
          <div>
            ...
            <div className="flex flex-col gap-2">
              <a href="https://www.playstation.com/" target="_blank" rel="noopener noreferrer" className="font-body text-sm text-text-secondary hover:text-gold-bright transition-colors focus-gold">{t('footer.official_site')}</a>
              <a href="https://insomniac.games/" target="_blank" rel="noopener noreferrer" className="font-body text-sm text-text-secondary hover:text-gold-bright transition-colors focus-gold">Insomniac Games ↗</a>
            </div>
            ...
            <div className="mt-4">
              <button onClick={toggleNoirMode} className="font-ui text-xs tracking-wider text-text-muted-custom hover:text-gold-mid transition-colors focus-gold" aria-label="Toggle noir mode">
                {noirMode ? t('footer.dark_mode') : t('footer.noir')}
              </button>
            </div>
          </div>
        </div>
