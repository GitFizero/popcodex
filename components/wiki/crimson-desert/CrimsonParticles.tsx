...
const CrimsonParticles = memo(({ count = 40 }: { count?: number }) => {
  ...
  useEffect(() => {
    ...
    const createParticle = (): Particle => ({
      ...
      lifetime: 0, maxLifetime: 240 + Math.random() * 240, opacity: 0,
    });
    ...
    const animate = () => {
      ...
      for (let i = particles.length - 1; i >= 0; i--) {
        ...
        ctx.fill();
        if (p.lifetime >= p.maxLifetime || p.y < -10) particles[i] = createParticle();
      }
      ctx.globalAlpha = 1;
    ...
    return () => { cancelAnimationFrame(animRef.current); window.removeEventListener('resize', debouncedResize); clearTimeout(resizeTimeout); };
  }, [count]);

  return <canvas ref={canvasRef} className="fixed inset-0 pointer-events-none" style={{ zIndex: 1, willChange: 'contents' }} aria-hidden="true" />;
});

CrimsonParticles.displayName = 'CrimsonParticles';
export default CrimsonParticles;
