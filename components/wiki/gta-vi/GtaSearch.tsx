...
const GtaSearch = ({ open, onClose, locale }: Props) => {
  ...
  return (
    <div className="fixed inset-0 z-[2000] bg-[hsl(220,67%,2%,0.97)] flex flex-col items-center pt-[20vh]" onClick={onClose}>
      <div className="w-full max-w-2xl px-4" onClick={e => e.stopPropagation()}>
        <div className="relative mb-6">
          ...
          <input
            ...
            placeholder="Search..."
            className="w-full bg-[hsl(213,33%,13%)] border border-neon-cyan/20 rounded-lg pl-12 pr-12 py-4 font-bebas text-2xl text-foreground placeholder:text-text-muted-vice focus:outline-none focus:border-neon-pink/50"
        ...
        <div className="space-y-1 max-h-[50vh] overflow-y-auto">
          {filtered.map(item => (
            <button
              ...
              className="w-full flex items-center gap-4 px-4 py-3 rounded-lg hover:bg-[hsl(213,33%,13%)] transition-colors text-left group"
            >
              <span className={`font-rajdhani text-[0.65rem] tracking-widest px-2 py-0.5 rounded bg-[hsl(214,36%,10%)] ${item.color}`}>
                {item.type}
              </span>
